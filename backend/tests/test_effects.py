...
def test_conditional_effect_then_branch():
    """Test that conditional effect executes 'then' branch when condition is true."""
    ...
    assert engine.state_manager.state.flags.get("high_health") is None
...
def test_conditional_effect_else_branch():
    ...
    assert engine.state_manager.state.flags.get("low_health") is None
    assert engine.state_manager.state.meters["player"]["health"] == 85  # 80 + 5


def test_random_effect_deterministic():
    """Test that random effect is deterministic with same seed."""
    game_def = create_test_game_def()

    # Create identical engines with same session (same seed)
    ...
    engine1._apply_random_effect(effect)
    engine2._apply_random_effect(effect)
...
def test_meter_multiply_divide():
    ...
    engine._apply_meter_change(multiply_effect)
    ...
    engine._apply_meter_change(divide_effect)
    ...
    engine._apply_meter_change(divide_zero_effect)
...
def test_nested_conditional_effects():
    ...
    # Apply the effect
    engine._apply_conditional_effect(nested_effect)

    # Both conditions should be true
    assert engine.state_manager.state.flags.get("both_conditions_true") is True
    assert engine.state_manager.state.flags.get("only_first_true") is None
    assert engine.state_manager.state.flags.get("first_false") is None


if __name__ == "__main__":
    test_conditional_effect_then_branch()
    test_conditional_effect_else_branch()
    test_random_effect_deterministic()
    test_meter_multiply_divide()
    test_nested_conditional_effects()
    print("✅ All effect processing tests passed!")